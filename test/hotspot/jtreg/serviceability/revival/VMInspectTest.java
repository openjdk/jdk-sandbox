...
import jdk.test.lib.dcmd.CoredumpJcmdExecutor;
...
public class VMInspectTest {
    ...
    public void run(CommandExecutor executor) throws ClassNotFoundException {
        ...
        // Run the test work method in this process, unless executor is working on a core file:
        if (!(executor instanceof CoredumpJcmdExecutor)) {
            TestWorkerClass test = new TestWorkerClass();
            test.work();
        }
    ...
    */
    ...
    public void core() throws Throwable {
        ProcessBuilder pb = ProcessTools.createTestJavaProcessBuilder("-XX:+CreateCoredumpOnCrash", "-Xmx128m",
                            "-XX:MaxMetaspaceSize=64m", "-XX:+CrashOnOutOfMemoryError", TestWorkerClass.class.getName());
        ...
        run(new CoredumpJcmdExecutor(coreFileName));
    }
}
...
class TestWorkerClass {
    ...
    // Test work meethod invoked directly when run in same process.
    public void work() {
        Runnable r = () -> {
            System.out.println("TestWorkerClass - in work method...");
            synchronized(lock) {
                try {
                    lock.wait();
                } catch (Exception e) { }
    ...
    public static void main(String [] args) {
        // For running in a new process. Call the same work method:
        TestWorkerClass test = new TestWorkerClass();
        test.work();

        // Then busywork to cause the OOM crash:
