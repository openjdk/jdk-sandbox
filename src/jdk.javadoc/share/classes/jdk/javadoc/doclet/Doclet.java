...
/// [package description][jdk.javadoc.doclet##package-description].
...
///
/// 1. The tool will create an instance of a doclet using the no-arg constructor
///    of the doclet class.
/// 2. Next, the tool calls the [`init`][#init] method with an appropriate locale
///    and reporter.
/// 3. Afterwards, the tool calls [`getSupportedOptions`][#getSupportedOptions],
///    and [`getSupportedSourceVersion`][#getSupportedSourceVersion].
///    These methods are only called once.
/// 4. As appropriate, the tool calls the [`run`][#run] method on the doclet
///    object, giving it a DocletEnvironment object, from which the doclet can
///    determine the elements to be included in the documentation.
///
...
///
/// To start the doclet, pass `-doclet` followed by the fully-qualified name of
/// the entry point class (i.e. the implementation of this interface) on the
/// javadoc tool command line.
///
/// @since 9
