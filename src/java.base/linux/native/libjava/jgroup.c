...
JNIEXPORT jint JNICALL
Java_com_alibaba_tenant_NativeDispatcher_createCgroup0(JNIEnv* env,
                                                       jclass clazz,
                                                       jstring group_path) {
    int res = 0;
    int cg_index = 0;
    ...
    if (group_path == NULL) {
        ...
        if (path_str != NULL || strlen(path_str) > 0) {
            ...
            for (cg_index = 0; cg_index < CG_MAX_SUPPORTED; ++cg_index) {
                ...
                res = mkdir(path_buf,
                            S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);  // modes are copied from libcg
                if (res == 0) {
...
JNIEXPORT jint JNICALL
Java_com_alibaba_tenant_NativeDispatcher_attachTask0(JNIEnv* env, jclass clazz, jstring group_path) {
...
JNIEXPORT jint JNICALL
Java_com_alibaba_tenant_NativeDispatcher_getAliOSPrimaryVersion0(JNIEnv* env,
                                                                       jclass clazz) {
    struct utsname un;
    if (0 == uname(&un)) {
        if (NULL != strstr(un.release, ".alios7")) {
            return (jint)7;
        } else if (NULL != strstr(un.release, ".alios")) {
            if (NULL != strstr(un.release, ".el6")) {
                return (jint)6;
            } else if (NULL != strstr(un.release, ".el5")) {
                return (jint)5;
            }
        }
    }
