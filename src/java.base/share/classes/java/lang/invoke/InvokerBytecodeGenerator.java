...
class InvokerBytecodeGenerator {
        ...
        try {
            return Classfile.build(classDesc, List.of(Classfile.Option.generateStackmap(false)), new Consumer<ClassBuilder>() {
                ...
                public void accept(ClassBuilder clb) {
                    ...
                    clb.withSuperclass(INVOKER_SUPER_DESC);
                    clb.withVersion(CLASSFILE_VERSION, 0);
                    clb.with(SourceFileAttribute.of(clb.constantPool().utf8Entry(SOURCE_PREFIX + name)));
                    config.accept(clb);
                }
            });
    ...
    private void emitAstoreInsn(CodeBuilder cob, int index) {
        emitStoreInsn(cob, L_TYPE, index);
    }
    ...
    private ClassDesc getInternalName(Class<?> c) {
    ...
    void emitStaticInvoke(CodeBuilder cob, MemberName member, Name name) {
        ...
        ClassDesc cname = getInternalName(defc);
    ...
    private Name emitSelectAlternative(CodeBuilder cob, Name selectAlternativeName, Name invokeBasicName) {
        ...
        emitAstoreInsn(cob, receiver.index());  // store the MH in the receiver slot
    ...
    private Name emitTryFinally(CodeBuilder cob, int pos) {
        ...
        MethodTypeDesc cleanupDesc = MethodTypeDesc.ofDescriptor(cleanupType.basicType().toMethodDescriptorString());
        ...
        if (isNonVoid) {
            emitStoreInsn(cob, basicReturnType, index);
        }
    ...
    private Name emitTableSwitch(CodeBuilder cob, int pos, int numCases) {
        ...
        for (int i = 0; i < numCases; i++) {
            ...
            emitLoadInsn(cob, L_TYPE, casesLocal);
    ...
    private Name emitLoop(CodeBuilder cob, int pos) {
        ...
        for (int c = 0, state = 0; c < nClauses; ++c) {
            ...
            if (cInitType.returnType() != void.class) {
                emitStoreInsn(cob, BasicType.basicType(cInitType.returnType()), firstLoopStateIndex + state);
                ++state;
            }
        }
        ...
        for (int c = 0, state = 0; c < nClauses; ++c) {
            ...
            emitLoopHandleInvoke(cob, invoker, steps, c, args, true, stepType, loopLocalStateTypes, clauseDataIndex,
                    firstLoopStateIndex);
            if (!isVoid) {
                emitStoreInsn(cob, BasicType.basicType(stepType.returnType()), firstLoopStateIndex + state);
                ++state;
            }
    ...
    private void emitLoopHandleInvoke(CodeBuilder cob, Name holder, int handles, int clause, Name args, boolean pushLocalState,
        ...
        if (pushLocalState) {
            for (int s = 0; s < loopLocalStateTypes.length; ++s) {
                emitLoadInsn(cob, BasicType.basicType(loopLocalStateTypes[s]), firstLoopStateSlot + s);
            }
        }
    ...
    private void emitPushArgument(CodeBuilder cob, Class<?> ptype, Object arg) {
        ...
        if (arg instanceof Name) {
            Name n = (Name) arg;
            emitLoadInsn(cob, n.type, n.index());
    ...
    private void emitStoreResult(CodeBuilder cob, Name name) {
        if (name != null && name.type != V_TYPE) {
            ...
            emitStoreInsn(cob, name.type, name.index());
        }
    }
    ...
    private void emitReturn(CodeBuilder cob, Name onStack) {
        ...
        if (rtype == V_TYPE) {
            ...
            if (rn != onStack) {
                emitLoadInsn(cob, rtype, lambdaForm.result);
            }
