...
public class IPAddressUtil {
    ...
    /**
     * Converts literal IP address string to an InetAddress with a use of supplied host name.
     *
     * @param addressString literal IP address {@code String}
     * @param hostName      host name to use for {@code InetAddress} construction
     * @return constructed {@code InetAddress}, or {@code null} if address string is not a literal
     * IP address string.
     */
    public static InetAddress toInetAddress(String addressString, String hostName) {
        if (IPAddressUtil.isIPv4LiteralAddress(addressString) || IPAddressUtil.isIPv6LiteralAddress(addressString)) {
            try {
                var bytes = InetAddress.getByName(addressString).getAddress();
    ...
    /*
     * Utility routine to check if the byte array contains an
